...
fn open_and_in_memory_run_migrations_automatically() {
    ...
    assert_eq!(schema_version(&file_store.connection()).unwrap(), 4);
    assert_eq!(schema_version(&memory_store.connection()).unwrap(), 4);
}
...
fn rollback_to_current_version_is_a_no_op() {
    ...
    rollback_to(&connection, 4).unwrap();
...
fn rollback_rejects_unknown_target() {
    ...
    let result = rollback_to(&connection, 4);
