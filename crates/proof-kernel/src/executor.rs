...
use crate::registry::{Governance, Registry, RegistryError};
...
impl ExecutionEngine {
    ...
    pub fn execute(
        ...
        let entry = self
            ...
            .ok_or_else(|| ExecutionError::OperationNotFound {
    ...
    pub fn is_agent_executable(&self, operation: &str, version: &str) -> Result<bool, ExecutionError> {
        let entry = self
            .registry
            .find(operation, version)
            .ok_or_else(|| ExecutionError::OperationNotFound {
...
pub fn create_proof(
    ...
    let input_canonical = crate::canonical::canonicalize(input)
        .map_err(|_| ProofError::Canonicalization)?;
    let output_canonical = crate::canonical::canonicalize(output)
        .map_err(|_| ProofError::Canonicalization)?;
    let input_digest = crate::canonical::digest(crate::canonical::ArtifactKind::OperationInput, &input_canonical);
    let output_digest = crate::canonical::digest(crate::canonical::ArtifactKind::OperationOutput, &output_canonical);
...
mod tests {
    use super::*;
    use crate::identity::{generate_keypair, Keypair};
    use serde_json::json;
    ...
    fn test_registry_entry(operation: &str, governance: Governance) -> crate::registry::RegistryEntry {
    ...
    fn executes_registered_operation() {
        let entries = vec![test_registry_entry("test.echo", Governance::AgentExecutable)];
        let engine = test_engine(entries);
        let context = test_context();
        let result = engine.execute("test.echo", "v1", &json!({"msg": "hello"}), &context).unwrap();
    ...
    fn rejects_unknown_operation() {
        ...
        assert!(matches!(result, Err(ExecutionError::OperationNotFound { .. })));
    }

    #[test]
    fn rejects_human_only_for_agents() {
        let entries = vec![test_registry_entry("test.human_only", Governance::HumanOnly)];
        let engine = test_engine(entries);
        let context = test_context();
    ...
    fn rejects_operation_without_handler() {
        let entries = vec![test_registry_entry("test.no_handler", Governance::AgentExecutable)];
        let engine = test_engine(entries);
        let context = test_context();
    ...
    fn create_proof_signs_correctly() {
        ...
        let proof = create_proof(actor, None, "test.op", &input, &output, Utc::now(), &keypair).unwrap();
