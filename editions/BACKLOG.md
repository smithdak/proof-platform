...
Gate A was accepted in D-E0002-011. Current authority is limited to E0002-13
freezing the operator contract/schema/evaluation and exact Gate B digest
packet; no implementation, migration, provider run, browser ceremony, or
external effect is authorized. After a separate Gate B, kernel,
independent auth, and mechanical fixtures may fan out in parallel. Storage
and runtime-core follow kernel in parallel while the loopback control shell
follows auth; a distinct backend conformance task then composes their real
SQLite/restart/fence/budget behavior. Authenticated HTTP reads may proceed from
completed storage/auth/control APIs while that integration runs; mutations
remain blocked on it. UI follows the protected APIs, then a later control-crate
owner assembles the real router/store/runtime/static app before independent
verification. The two HTTP waves use one named crate owner and are strictly
sequential. Lockfile and generated-app writes are explicitly serialized.
Every product/evaluation path uses a disposable trusted workspace and fresh
identities; the compromised repository-root `.proof` identity is prohibited.
...
| E0002-06 | Persist filtered multi-run projections and an atomic append-only operator-command ledger | E0002-05 | `crates/proof-storage/**` | Migration 14 and reopen round trips pass against the authoritative trusted store; bounded keyset pagination has no skips/duplicates; cancel/complete, lease/reclaim, stale-fence, command replay, and aggregate-budget races linearize exactly | critical | `gpt-5.6-sol` |
| E0002-07 | Enforce runtime cancel/resume, fenced recovery, and aggregate-budget barriers against kernel stores | E0002-05 | `crates/proof-agent-runtime/**` | Recording-store barrier tests prove cancel-before-dispatch has zero provider/tool effect, cancel/resume has one winner, and stale epochs cannot dispatch/write; durable SQLite acceptance remains E0002-14 | critical | `gpt-5.6-sol` |
| E0002-11 | Build the loopback operator-control launcher, signed-challenge session, protected synthetic assembly/store-opener shell, revoke, and shutdown | E0002-08 | `crates/proof-operator-control/**` plus explicitly delegated W4 root manifest/lock delta | Process/Tower tests prove clean loopback delivery, independent issuance, no W4 storage/runtime dependency, no legacy route fallback, no persistence, explicit revoke, control-plane restart invalidation, and fail-closed shutdown | critical | `gpt-5.6-sol` |
| E0002-14 | Independently integrate real SQLite, runtime barriers, and the control shell | E0002-06,07,11 | `conformance/**` plus sole W5 `Cargo.lock` ownership after package-manifest freeze | Restart/reopen, monotonic fences, command idempotency, aggregate budgets, worker/control restart separation, authoritative store, and zero-effect races pass before mutations | critical | `gpt-5.6-sol` |
...
| E0002-09 | Add authenticated approve/deny, cancel, and explicit-resume APIs | E0002-02, E0002-07, E0002-08, E0002-11, E0002-14 | `crates/proof-transport-http/**` in a later sequential wave | Deterministic mutation races preserve one decision/command winner, no automatic resume, no key/provider/write after lost authority, expected revisions, idempotent receipts, and uncertain-response recovery | critical | `gpt-5.6-sol` |
| E0002-03 | Build the multi-run operator console against frozen schemas and scoped APIs | E0002-02, E0002-09, E0002-11 | `apps/operator-console/**` | Browser evidence covers attention filters, exact run/approval/audit detail, distinct decision/cancel/session-revoke challenges, accessibility, stale controls, session loss, uncertain-write recovery, no credential persistence, and a frozen reproducible static-bundle digest | high | `gpt-5.6-terra` |
| E0002-15 | Assemble the real protected router, trusted store/runtime, independent session, and frozen static app in the runnable control process | E0002-02,03,09,11,14 | later sequential `crates/proof-operator-control/**`; `Cargo.lock`; generated `apps/operator-console/dist/**` only | Process/Tower inventory proves the real product uses one authoritative store/signer and protected router with no legacy fallback, alternate path, ephemeral signer, embedded credential, or UI source drift | critical | `gpt-5.6-sol` |
| E0002-04 | Independently verify API/browser concurrency, recovery, authority, and zero-effect boundaries | E0002-02, E0002-03, E0002-07, E0002-08, E0002-09, E0002-11, E0002-12, E0002-14, E0002-15 | verifier-only dogfood evidence and unique handoff | Frozen all-required evaluation passes exact auth, pagination, decision, cancel/resume, lease/recovery, budget, browser secrecy, audit chronology, real product assembly, separate worker/control-plane restart, and sentinel vectors | critical | `gpt-5.6-sol` |
| E0002-10 | Quiescent integration and product-owner Gate C | E0002-01,02,03,04,05,06,07,08,09,11,12,13,14,15 | root integration, public guidance, edition release records | Scoped/reverse-impact and quiescent gates pass; limitations, rollback, spend, independent verdict, and dated owner accept/defer/reject are recorded without claiming E0006 release evidence | high | `gpt-5.6-sol` |
