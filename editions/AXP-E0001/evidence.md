...
|---|---|---|---|---|
| Edition structure | `rtk scripts/swarm.sh validate AXP-E0001` | valid | revised fourteen-task W1-W11 graph passed after the E0001-14 authority repair; edition diff check clean on 2026-08-30 | passed |
...
| Delegation principal integrity | E0001-14 focused and compile gates | distinct-recipient grant preserves workspace Agent ID/kind/key; enrolled Human recipient is rejected unchanged before delegation persistence | focused delegation module 5/5; all CLI test targets compile; formatting and scoped diff pass. Managed-sandbox full run reached 44 passes, then only the known one loopback and fourteen synthetic-root ancestry failures; host-context full replay is mandatory before W10 provider construction | passed with host replay barrier |
