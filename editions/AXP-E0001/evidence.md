...
|---|---|---|---|---|
| Edition structure | `rtk scripts/swarm.sh validate AXP-E0001` | valid | revised fifteen-task W1-W12 graph passed after the E0001-15 authority repair; edition diff check clean on 2026-08-30 | passed |
...
| CLI impact | `rtk scripts/test-scoped.sh proof-transport-cli`; post-E0001-15 complete host replay | CLI passes with preflight-before-secret tests | historical final rerun passed 68 plus 11 child tests; current complete host replay passes 72/72 across two suites after both authority repairs | passed |
| Delegation principal integrity | E0001-14 focused and compile gates | distinct-recipient grant preserves workspace Agent ID/kind/key; enrolled Human recipient is rejected unchanged before delegation persistence | focused delegation module 5/5; all CLI test targets compile; formatting and scoped diff pass. Managed-sandbox full run reached 44 passes before known environment barriers; current host suite passes 72/72 and exact readiness replay remains stable | passed |
| Workspace import identity/authority integrity | E0001-15 focused rejection, compatibility, and compile gates | reject kind/key replacement, delegation replacement/unrevocation, and invalid-signature partial principal/proof persistence while preserving compatible imports | rejection 3/3; archive round-trip 1/1; newer-proof rule 1/1; all CLI targets compile; formatting passes. Managed-sandbox full run reached 46 passes before known environment barriers; current host suite passes 72/72 | passed |
...
| Post-repair readiness replay | host `live-prepare finish` with both provider variables unset after E0001-14/E0001-15 | exact retained 10/10 evidence, bindings, `next_argv`, and binding digest; no provider boundary | exact packet and digest `7efebe8d98ad23f9a122ae64f76887d371804f719bae7fb4a69b434e511b6b71` returned; ready record remains mode `0600` under workspace mode `0700`; zero provider attempt | passed |
