...
- [x] E0001-20 live execution now combines a crash-released one-writer lease with expected-tail checkpoint CAS, exact prior-state/epoch fencing, and prospective-history validation; resume/stale-CAS contention is zero-write, start contention is bounded to its atomic initial four records, and one provider send remains exact
...
| Exact live-start replay could allocate a second paid run; follow-ups lost workspace/policy/Human bindings; post-claim crash stranded the sole pristine run; concurrent resumers could poison immutable history or orphan a paid response; no secure live-v2 runbook existed | resolved in E0001-20 with an atomic four-record start claim, provider-free Existing replay, exact pristine same-run recovery, crash-released one-writer lease, expected-tail checkpoint CAS, emitted bound argv, and a fail-closed operator procedure | E0001-20 | Kernel 106/106, Storage 133/133, Runtime 122/122, host CLI 93/93, reverse impact 613/613, independent runtime/CLI/runbook/concurrency audits, and exact final-source readiness replay pass |
